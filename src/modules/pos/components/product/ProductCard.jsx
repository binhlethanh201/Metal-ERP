...
const ProductCard = ({
  ...
  if (horizontal) {
    return (
      <button
        ...
        className="group flex w-full items-center gap-3 overflow-hidden rounded-lg border border-slate-200 bg-white text-left transition-all hover:shadow-md active:scale-[0.99] dark:border-[#333333] dark:bg-[#0f0f0f]"
      >
        {product.image ? (
          <img
          ...
          <div className="flex h-[80px] w-[80px] shrink-0 items-center justify-center bg-slate-100 text-lg font-bold text-slate-300 dark:bg-[#1a1a1a] dark:text-[#808080]">
        ...
        <div className="flex min-w-0 flex-1 items-center justify-between gap-2 pr-3">
          <div className="min-w-0">
            ...
            <div className="mt-1 flex items-center gap-2">
              <span
                className={`text-[11px] font-bold ${product.stock <= 5 ? 'text-orange-600' : 'text-slate-500 dark:text-[#999999]'}`}
              >
                SL: {product.stock}
              </span>
              <span
          ...
          <div className="shrink-0 text-right">
            ...
            <div className="text-xs text-slate-400 dark:text-[#808080]">/ {product.unit || 'Cái'}</div>
          </div>
        </div>
      </button>
    );
  ...
  return (
    <button
      onClick={handleClick}
      disabled={outOfStock || disabled}
      className="group relative flex h-[410px] flex-col overflow-hidden rounded-xl border border-slate-200 bg-white text-left transition-all hover:shadow-lg active:scale-95 dark:border-[#333333] dark:bg-[#0f0f0f]"
    >
      {product.image ? (
        <img
        ...
        <div className="flex h-[230px] w-full shrink-0 items-center justify-center bg-slate-100 text-4xl font-bold text-slate-300 dark:bg-[#1a1a1a] dark:text-[#808080]">
          {product.name?.charAt(0) || '?'}
        </div>
      )}
      ...
      <div className="flex h-[180px] flex-col justify-between p-4">
        ...
        <div className="mt-auto pt-3">
          ...
          <div className="mt-2 flex items-center justify-between text-sm text-slate-500 dark:text-[#999999]">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-[#808080]">
              {product.sku}
            </span>
            <span className="text-xs font-semibold text-slate-600 dark:text-[#999999]">
              SL: {product.stock}
              <span className="ml-1 text-slate-400 dark:text-[#808080]">({product.unit || 'Cái'})</span>
            </span>
          </div>
        </div>
      </div>
