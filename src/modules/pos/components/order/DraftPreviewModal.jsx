...
const printDraft = (draft) => {
  ...
  const itemsHtml = (draft.items || [])
    .map(
      (item) => `
      <tr>
        <td class="text-left">${item.name || item.productName || 'Sản phẩm'}</td>
        <td class="text-center">${item.quantity || 0}</td>
        <td class="text-center">${cleanUnit(item.displayUnit || item.selectedUnit || item.unit || '')}</td>
        <td class="text-right">${formatCurrency((item.price || item.unitPrice || 0) * (item.quantity || 0))}</td>
      </tr>`
    )
    .join('');

  const total = (draft.items || []).reduce((sum, item) => sum + (item.price || item.unitPrice || 0) * (item.quantity || 0), 0);
  const customerName = draft.customer ? (draft.customer.name || draft.customer) : 'Khách lẻ';
...
<html lang="vi">
<head><meta charset="utf-8"><title>Nháp hóa đơn ${draft.id?.replace?.('draft-', '') || Date.now()}</title>
<style>
...
<div class="text-center">
  ...
  <div class="fs-sm">(NHÁP)</div>
  <div>Mã: NH${draft.id?.replace?.('draft-', '') || Date.now().toString().slice(-8)}</div>
  <div class="fs-sm">${new Date().toLocaleString('vi-VN')}</div>
</div>
<hr>
<div class="flex-between"><span>Khách hàng:</span><span>${customerName}</span></div>
<hr>
<table>
...
<div class="text-center" style="margin-top:12px">
  <div class="fs-sm">Lưu ý: Đây là nháp, chưa finalize.</div>
</div>
...
const DraftPreviewModal = ({ isOpen, onClose, draft }) => {
  if (!draft) return null;

  const subtotal = (draft.items || []).reduce((sum, item) => sum + (item.price || item.unitPrice || 0) * (item.quantity || 0), 0);
  const customerName = draft.customer ? (draft.customer.name || draft.customer) : 'Khách lẻ';
    ...
    >
      <div className="space-y-4 text-base">
        <div className="text-center border-b pb-3 dark:border-slate-700">
          ...
          <p className="text-sm mt-1 font-semibold text-amber-600 dark:text-amber-400">(NHÁP)</p>
          <p className="text-sm font-semibold text-slate-600 dark:text-[#999999]">Mã: {draft.id?.replace?.('draft-', '') || 'N/A'}</p>
        ...
        <div className="border-t border-b py-3 dark:border-slate-700">
          ...
          {draft.items.map((item, idx) => (
            <div key={idx} className="grid grid-cols-4 gap-2 py-1.5 text-sm text-slate-700 dark:text-[#b3b3b3] border-b border-slate-100 dark:border-slate-800 last:border-0">
              <span className="break-words">{item.name || item.productName || 'Sản phẩm'}</span>
        ...
        <p className="text-center text-xs text-slate-400 dark:text-[#808080] italic">Lưu ý: Đây là nháp, chưa finalize.</p>
      </div>
    </Modal>
  );
};
