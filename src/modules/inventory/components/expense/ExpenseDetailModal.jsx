  ...
  const detailTitle = detailVoucher ? (
    <div className="flex flex-col gap-1">
      ...
      <div className="text-sm font-normal text-slate-500">
        Chi nhánh: <strong className="text-slate-700">{detailVoucher.branchName}</strong>
      </div>
    </div>
  ) : (
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={detailTitle} size="lg" footer={modalFooter}>
      {loading ? (
        ...
        <div className="space-y-5">
          {cancelMode && (
            <div className="animate-fade-in rounded-lg border border-red-200 bg-red-50 p-4 shadow-sm">
          ...
          <div className="grid grid-cols-1 gap-4 rounded-lg border border-slate-200 p-4 sm:grid-cols-2">
            <div>
              <div className="text-xs font-medium uppercase text-slate-400">Nhóm chi phí</div>
              <div className="mt-0.5 text-sm font-semibold text-slate-800">
                {detailVoucher.categoryName}
              </div>
            </div>
            <div>
              <div className="text-xs font-medium uppercase text-slate-400">Nhà cung cấp</div>
              <div className="mt-0.5 text-sm font-semibold text-slate-800">
                {detailVoucher.supplierName || '---'}
              </div>
            </div>
            <div>
              <div className="text-xs font-medium uppercase text-slate-400">Số tiền</div>
              <div className="mt-0.5 text-base font-bold text-emerald-600">
                {formatCurrency(detailVoucher.amount)}
              </div>
            </div>
            <div>
              <div className="text-xs font-medium uppercase text-slate-400">Người tạo</div>
              <div className="mt-0.5 text-sm font-semibold text-slate-800">
                {detailVoucher.createdByName || '---'}
              </div>
            </div>
            <div>
              <div className="text-xs font-medium uppercase text-slate-400">Ngày tạo</div>
            ...
            {detailVoucher.note && (
              <div className="border-t border-slate-100 pt-3 sm:col-span-2">
                <div className="text-xs font-medium uppercase text-slate-400">Ghi chú</div>
                <div className="mt-0.5 text-sm text-slate-600">{detailVoucher.note}</div>
              </div>
            )}
          </div>
          ...
          {detailVoucher.status === 'CANCELLED' && (
            <div className="rounded-lg border border-rose-100 bg-rose-50 p-4">
              <div className="text-xs font-semibold uppercase text-rose-700">Thông tin hủy</div>
