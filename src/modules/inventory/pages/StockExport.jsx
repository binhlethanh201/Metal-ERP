...
export const StockExport = () => {
  ...
  const loadData = async (filterParams = {}) => {
    ...
    try {
      ...
      try {
        const suppliersResponse = await getSuppliers({ pageNumber: 1, pageSize: 200 });
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      setGlobalError(err.message || 'Không thể tải template');
    }
  };
    ...
    try {
      ...
      if (data?.success) {
        ...
        if (errors && errors.length > 0) {
          // Có lỗi -> hiện preview trong modal để user xem
          setImportResult(data.data);
        }
        // Đổ dữ liệu valid vào form items
