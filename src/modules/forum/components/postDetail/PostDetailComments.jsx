...
const PostDetailComments = ({ comments: initialComments = [] }) => {
  ...
  const [reportTarget, setReportTarget] = useState('bài viết');
  ...
  return (
    <>
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        {/* Engagement bar - liền khung với bình luận */}
        <div className="flex flex-wrap items-center gap-2 border-b border-slate-100 px-5 py-3">
          <button
            type="button"
            onClick={handleLike}
            className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-bold transition-colors ${liked ? 'bg-blue-100 text-[#004785]' : 'text-slate-500 hover:bg-blue-50 hover:text-[#004785]'}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-sm font-bold transition-colors ${disliked ? 'bg-red-50 text-red-600' : 'text-slate-400 hover:bg-red-50 hover:text-red-500'}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-bold transition-colors ${saved ? 'text-amber-600' : 'text-slate-500 hover:text-amber-600'}`}
          >
          ...
          <button
            type="button"
            onClick={() => {
              setReportTarget('bài viết');
              setReportOpen(true);
            }}
            className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-bold text-slate-400 transition-colors hover:text-red-500"
          >
      ...
      <ReportModal isOpen={reportOpen} onClose={() => setReportOpen(false)} target={reportTarget} />
    </>
  );
};
