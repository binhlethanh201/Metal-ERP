...
const AdminNotificationDropdown = () => {
  ...
  const getIconForTarget = (target) => {
    switch ((target || '').toUpperCase()) {
      case 'ALL': case 'ALL_USERS': return { name: 'public', color: 'text-blue-500 dark:text-blue-400', bg: 'bg-blue-100 dark:bg-blue-900/30' };
      case 'OWNER': return { name: 'storefront', color: 'text-amber-500 dark:text-amber-400', bg: 'bg-amber-100 dark:bg-amber-900/30' };
      case 'STAFF': return { name: 'badge', color: 'text-purple-500 dark:text-purple-400', bg: 'bg-purple-100 dark:bg-purple-900/30' };
  ...
  const dropdownPanel = isOpen && (
    <div
      ref={panelRef}
      style={panelStyle}
      className="w-[380px] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl dark:border-[#333333] dark:bg-[#0f0f0f]"
    >
            ...
            {notifications.map((notif) => {
              ...
              return (
                ...
                >
                  ...
                  {unread && <div className="mt-2 h-2.5 w-2.5 shrink-0 rounded-full bg-blue-600 shadow-sm" />}
                </div>
              );
            })}
          </div>
